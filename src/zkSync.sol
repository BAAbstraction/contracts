...
/**
 * This is contract for zkSync.
 * We use zkSolc to deploy manually
 * `zksolc src/zkSync.sol --asm --bin`
 *
 * pls refer to ./zkSync/bytecode file with bytecode 
 */
...
contract IntermediateFactory is Ownable {
  ...
  function deploySafeClone(
    ...
    address _singleton = 0xB00ce5CCcdEf57e539ddcEd01DF43a13855d9910; // 1.3.0 for zkSync
    SafeProxy safe = new SafeProxy(_singleton);
    SafeInterface(address(safe)).setup(_owners, _threshold, address(0), bytes(''), address(0), address(0), 0, payable(address(0)));
    return address(safe);
  }
}
...
contract NFTOptionsZksync is ERC721Enumerable, Constants {
